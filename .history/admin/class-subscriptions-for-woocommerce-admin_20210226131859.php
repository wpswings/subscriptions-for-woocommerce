...
class Subscriptions_For_Woocommerce_Admin {
	...
	public function sfw_admin_enqueue_styles( $hook ) {
		...
		if ( isset( $screen->id ) && ('makewebbetter_page_subscriptions_for_woocommerce_menu' == $screen->id ||  $screen->id == 'product' ) ) {
	...
	public function mwb_sfw_custom_product_fields_for_subscription(){
		...
		<p class="form-field mwb_sfw_subscription_free_trial_field ">
		 ...
		 <?php //echo wc_help_tip('test'); ?>
			...
			$sfw_general_html = $sfw_mwb_sfw_obj->mwb_sfw_plug_generate_html( $sfw_settings_template );
		echo esc_html( $sfw_general_html );
					// woocommerce_wp_text_input( array(
					// 	'id'                => 'mwb_sfw_subscription_price',
					// 	'value'             => get_post_meta( get_the_ID(), 'mwb_sfw_subscription_price', true ),
					// 	'label'     		=> __( 'Subscriptions Price', 'subscriptions-for-woocommerce' ) . ' (' . get_woocommerce_currency_symbol() . ')',
					// 	'data_type' 		=> 'price',
					// 	'description'       => __('Set the subscriptions price.','subscriptions-for-woocommerce'),
					// 	'desc_tip'			=> true
					// ) );
