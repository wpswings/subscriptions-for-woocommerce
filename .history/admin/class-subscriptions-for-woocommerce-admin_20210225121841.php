...
class Subscriptions_For_Woocommerce_Admin {
	...
	*/
	...
	public function save_custom_fields_for_single_products( $post_id) {
		...
		//update_post_meta( $post_id,'_mwb_sfw_product',$_POST['_mwb_sfw_product'] );
		$product->update_meta_data( '_mwb_sfw_product', $_POST['_mwb_sfw_product'] );
			$key = '_mwb_sfw_product';
			//$product->post->$key = 'yes';
		
			$product->$key = 'yes';
		
		$is_wc_data = $product instanceof WC_Data;
		var_dump($is_wc_data);
		$product->save();
		die('-->>');
	}
