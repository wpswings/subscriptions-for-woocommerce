...
class Subscriptions_For_Woocommerce_Admin {
	...
	public function sfw_admin_enqueue_scripts( $hook ) {
		...
		if ( isset( $screen->id ) && 'product' == $screen->id ) {
			wp_register_script( 'mwb-sfw-admin-single-product-js', SUBSCRIPTIONS_FOR_WOOCOMMERCE_DIR_URL . 'admin/src/js/subscriptions-for-woocommerce-admin.js', array( 'jquery' ), $this->version, false );
			wp_enqueue_script( $this->plugin_name . 'admin-js' );
		}
	}
	...
	*/
	...
	public function mwb_sfw_custom_product_fields_for_subscription( $tabs ){
		$tabs['mwb_sfw_product'] = array(
			'label'    => 'Subscriptions settings',
			'target'   => 'mwb_sfw_product_target_section',
			'class'    => array( 'hide_if_simple', 'hide_if_variable', 'hide_if_virtual', 'hide_if_grouped', 'hide_if_external' ),
			'priority' => 80,
		);
		return $tabs;
	}
