...
class Subscriptions_For_Woocommerce_Public {
	...
	public function mwb_sfw_subscription_product_get_price_html( $price, $product ) {
		...
		if( is_object( $product ) ) {
			...
			// elseif( isset( $mwb_sfw_subscription_initial_signup_price ) && ! empty( $mwb_sfw_subscription_initial_signup_price ) ) {
			// 	die('-->>jfsdjkfldsfdfg');
			// 	$price .= ' and ' . wc_price( $mwb_sfw_subscription_initial_signup_price ) .'Sign up fee';
			// }
	...
	public function mwb_sfw_get_free_trial_period_html( $product_id, $price ){
		...
		if( isset( $mwb_sfw_subscription_free_trial_number ) && !empty( $mwb_sfw_subscription_free_trial_number ) ) {
			$mwb_price_html = $this->mwb_sfw_get_time_interval( $mwb_sfw_subscription_free_trial_number, $mwb_sfw_subscription_interval );
			$price .= __(' and ','subscriptions-for-woocommerce') . wc_price( $mwb_sfw_subscription_free_trial_number ) .__(' Sign up fee','subscriptions-for-woocommerce');
		}
		return $price;
	}
