...
class Subscriptions_For_Woocommerce_Public {
	...
	public function mwb_sfw_subscription_product_get_price_html( $price, $product ) {
		...
		if( is_object( $product ) ) {
			...
			if( isset( $mwb_sfw_subscription_number ) && ! empty( $mwb_sfw_subscription_number ) ) {
	...
	public function mwb_sfw_get_time_interval( $mwb_sfw_subscription_number, $mwb_sfw_subscription_interval ) {
		print_r($mwb_sfw_subscription_number);
		$text = sprintf( _n( '%s star', '%s stars', $mwb_sfw_subscription_number, 'subscriptions-for-woocommerce' ), $mwb_sfw_subscription_number );
		print_r($text);
		switch( $mwb_sfw_subscription_interval ){
			case 'days':
				$mwb_price_html .= _sprintf( _n( '%s Day', '%s Days', $mwb_sfw_subscription_number, 'subscriptions-for-woocommerce' ), $mwb_sfw_subscription_number );
				break;
			case 'week':
				$mwb_price_html .= _sprintf( _n( '%s Week', '%s Weeks', $mwb_sfw_subscription_number, 'subscriptions-for-woocommerce' ), $mwb_sfw_subscription_number );
				break;
			case 'month':
				$mwb_price_html .= _sprintf( _n( '%s Month', '%s Months', $mwb_sfw_subscription_number, 'subscriptions-for-woocommerce' ), $mwb_sfw_subscription_number );
				break;
			case 'year':
				$mwb_price_html .= _sprintf( _n( '%s Year', '%s Years', $mwb_sfw_subscription_number, 'subscriptions-for-woocommerce' ), $mwb_sfw_subscription_number );
				break;
		}
