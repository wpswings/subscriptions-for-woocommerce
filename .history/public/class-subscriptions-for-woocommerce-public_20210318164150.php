...
class Subscriptions_For_Woocommerce_Public {
	...
	public function mwb_sfw_set_susbcription_total( $total, $mwb_subscription ) {
		...
		if ( ! empty( $_POST['_mwb_sfw_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_mwb_sfw_nonce'] ) ), 'mwb_sfw__change_payment_method' ) && isset( $_POST['mwb_change_change_payment'] ) && $mwb_subscription->get_order_key() == $_GET['key'] && $mwb_subscription->get_id() == absint( $_POST['mwb_change_change_payment'] ) ) {
			$total = 0;
