			...
			if ( isset( $mwb_subscriptions ) && ! empty( $mwb_subscriptions ) && is_array( $mwb_subscriptions ) ) {
				foreach ( $mwb_subscriptions as $key => $value ) {
					...
					if ( mwb_sfw_check_valid_subscription( $subscription_id ) ) {
						...
						$check_ava_activate = in_array( 'woocommerce-avatax/woocommerce-avatax.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) );
						...
						if ( $check_ava_activate ) {
							wc_avatax()->get_order_handler()->calculate_order_tax( $mwb_new_order, false, true );
						}

						$parent_order_id = get_post_meta( $subscription_id, 'mwb_parent_order', true );
						$parent_order    = wc_get_order( $parent_order_id );
						$order_shipping  = $parent_order->get_items( 'shipping' );
						...
						foreach ( $order_shipping as $item_id => $item ) {
							...
							if ( $shipping_data_name && $shipping_data_total ) {
								...
								$shipping_fee->set_props(
									array(
										'method_title' => $shipping_data_name,
										'total'        => wc_format_decimal( $shipping_data_total ),
										'order_id'     => $mwb_new_order->get_id(),
									)
								);
								$shipping_fee->set_method_id( $shipping_data_method_id );
								$shipping_fee->set_instance_id( $shipping_data_instance_id );

								$mwb_new_order->add_item( $shipping_fee );
							}
						}
						if ( ! $check_ava_activate ) {
