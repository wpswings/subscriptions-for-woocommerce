<?php
/**
 * Membership Email — Expired
 *
 * Sent when a membership expires (fixed-length access-length reached, driven
 * by the daily cron). Triggered by `wps_membership_status_changed`
 * (new_status = expired).
 *
 * @since      2.0.0
...
if ( ! class_exists( 'WPS_Membership_Expired_Email' ) ) {
	 ...
	 * @since 2.0.0
	 */
	class WPS_Membership_Expired_Email extends WC_Email {
		...
		public function __construct() {
			$this->id             = 'wps_membership_expired';
			$this->title          = __( 'Membership Expired', 'subscriptions-for-woocommerce' );
			$this->description    = __(
				'Sent when a membership expires.',
				'subscriptions-for-woocommerce'
			);
			$this->template_html  = 'wps-membership-expired-email.php';
			$this->template_plain = 'plain/wps-membership-expired-email.php';
		...
		/**
		 * Fire only when the new status is "expired".
		 *
		 * @since 2.0.0
		...
		public function maybe_trigger( $user_id, $plan_slug, $new_status, $old_status ) {
			...
			$fallback    = array( 'status' => $new_status );
			$row         = isset( $memberships[ $plan_slug ] ) ? $memberships[ $plan_slug ] : $fallback;
		...
		public function get_default_subject() {
			return __( 'Your membership has expired — {site_title}', 'subscriptions-for-woocommerce' );
		}
		...
		public function get_default_heading() {
			return __( 'Membership Expired', 'subscriptions-for-woocommerce' );
		}
...
return new WPS_Membership_Expired_Email();
